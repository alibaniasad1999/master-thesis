...
\begin{itemize}
	...
	\item این رویکرد نیاز به دسترسی به منابع محاسباتی با عملکرد بالا برای آموزش یادگیری تقویتی دارد قبل از اجرای آن در رایانه پرواز فضاپیما.
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item پیاده‌سازی مدل دینامیکی محیط مورد نظر انجام می‌شود. این مدل باید شامل تمام جوانب مهم محیط رباشد، از جمله قوانین حرکت، ابعاد فضایی، و وضعیت اولیه.
		\item در صورت پیچیدگی مدل دینامیکی محیط، از کتابخانه‌ها و ابزارهای مهندسی نرم‌افزاری مانند NumPy و SciPy در پایتون برای پیاده‌سازی آن استفاده کرد.
	...
	\begin{itemize}
		...
		\item برای استفاده از \lr{GPU} در \lr{Google Colab}، می‌توانید از کتابخانه‌های معروفی مانند \lr{TensorFlow} یا \lr{PyTorch} که از \lr{GPU} پشتیبانی می‌کنند، استفاده می‌شود. به این ترتیب، آموزش مدل‌های یادگیری تقویتی بسیار سریع‌تر و رایگان انجام می‌شود.
	\end{itemize}
\end{itemize}
...
شبکه‌های عصبی ویژگی‌های جذابی را برای فعال‌سازی \lr{automatic guidance} دارند. به عنوان مثال، شبکه‌های عصبی می‌توانند به‌طور مستقیم از تخمین‌های وضعیت به دستورهای پیشران کنترلی که با محدودیت‌های مأموریت سازگار است، برسند. عملکرد هدایت شبکه‌های عصبی در مطالعاتی مانند فرود بر سیارات \cite{gaudet2020six}، عملیات نزدیکی به سیارات \cite{gaudet2020terminal} و کنترل فضاپیما با پیشران ازدست رفت \cite{rubinsztejn2020neural} نشان داده شده است.
...
به‌عنوان مثال، الگوریتم‌های یادگیری ماشینی ابتدایی در فضاپیماهای مریخی برای کمک به شناسایی ویژگی‌های زمین‌شناسی تعبیه شده‌اند. الگوریتم (AEGIS) توانایی انتخاب خودکار هدف \lr{ChemCam} در داخل فضاپیماهای \lr{Spirit}، \lr{Opportunity} و \lr{Curiosity} را فعال می‌کند
\cite{estlin2012aegis}.
  در کامپیوتر پرواز اصلی \lr{Curiosity}، فرآیند بهبود نقطه‌گیری \lr{(refinement process) } نیاز به 94 تا 96 ثانیه دارد 
  \lr{francis2017aegis},
...
علاوه بر رباتیک سیاره‌ای، تحقیقات مختلفی به تکنیک‌های مختلف یادگیری ماشینی برای مسائل astrodynamics اختصاص داده‌اند. در طراحی مسیر، وظایف رگرسیون معمولاً مؤثرتر هستند. به عنوان مثال، Dachwald در سال 2004 از یک شبکه عصبی کم‌عمق (NN) در بهینه‌سازی مسیرهای رانشگر کم پیشران استفاده کرد .\cite{dachwald2004evolutionary} تحقیقات به تازگی شامل شناسایی انتقال‌های هتروکلینیک \cite{desmet2019identifying}، اصلاح مسیر رانشگر کم پیشران\cite{parrish2018lowthrust} و تجزیه و تحلیل مشکلات ازدست رفتن رانشگر\cite{rubinsztejn2020neural} شده‌اند.
تکنیک‌های یادگیری نظارتی می‌توانند نتایج مطلوبی تولید کنند، اما دارای محدودیت‌های قابل توجهی هستند. ابتدا، چنین رویکردها فرآیند تصمیم‌گیری را فرض می‌کنند. با انتخاب نتایج مطلوب، کاربر فرض می‌کند که دانش از پیش از فرآیند تصمیم‌گیری را دارد. این فرضیه به معنای دقیق بودن داده‌های تولید‌شده توسط کاربر برای نتایج مطلوب است و همچنین به تکنیک‌های موجود برای حل مشکل کنونی و تولید داده نیاز دارد. در دامنه‌هایی که چنین دانشی وجود ندارد، تکنیک‌های یادگیری نظارتی قابل استفاده نیستند.
در سال‌های اخیر، RL به اثبات مفید بودن خود در دستیابی به عملکرد بهترین حالت در دامنه‌هایی با ابهام محیطی قابل توجه رسیده است
 \cite{heess2017emergence, silver2017mastering}. هدایت فعال‌سازی‌شده توسط RL به صورت گسترده‌ای بر اساس فاز پرواز دسته‌بندی می‌شوند. مسائل فرود \cite{furfaro2020adaptive, gaudet2020deep} و عملیات در نزدیکی به اجسام کوچک
...
\begin{table}[htbp]
	...
	\begin{tabular}{|P{3cm}|P{10cm}|}
		...
		\hline
		\multirow{3}{*}{\textbf{تابستان سال اول}} & تست \\
		\cline{2-2}
		& \\
		\cline{2-2}
		&  \\
		\hline
		\multirow{3}{*}{\textbf{نیمسال اول سال دوم}} & \textbf{الف) طراحی و توسعه سیستم ناوبری مشارکتی و تکنیک‌های ترازیابی در حین پرواز} \\
		\cline{2-2}
		& \\
		\cline{2-2}
		& \textbf{ج) شبیه‌سازی عملکرد سیستم ناوبری اینرسی مشارکتی در شرایط مختلف و ارزیابی عملکرد آن} \\
		\hline
		\multirow{3}{*}{\textbf{نیمسال دوم سال دوم}} & \textbf{الف) آموزش الگوریتم‌های هوش مصنوعی برای رهبر گروه و اعضای گروه با استفاده از داده‌های جمع‌آوری‌شده} \\
		\cline{2-2}
		& \textbf{ب) ارزیابی و تحلیل نتایج شبیه‌سازی‌ها و آزمایش‌ها در مورد دقت و قابلیت اطمینان سیستم ناوبری} \\
		\cline{2-2}
		& \textbf{ج) اصلاح و بهبود عملکرد سیستم ناوبری بر اساس نتایج به دست‌آمده} \\
		\cline{2-2}
		& \textbf{د) انجام پیاده‌سازی سخت‌افزاری سیستم ناوبری مشارکتی بر روی میکروپروسسورها و ارتباط با مدل‌های ریاضی پرنده‌ها} \\
		\cline{2-2}
		& \textbf{ه) نتیجه‌گیری از تحقیقات و ارائه گزارش نهایی شامل معرفی مسئله، روش‌های استفاده‌شده، نتایج حاصل و پیشنهادات برای تحقیقات آتی} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{latin}
	\begin{thebibliography}{1}
		...
		in: \textit{29th AAS/AIAA Space Flight Mechanics Meeting}, American Astronautical Society, Ka’anapali, Hawaii, 2019, pp. 1–18.
