...
\lr{TD3}\LTRfootnote{Twin Delayed Deep Deterministic Policy Gradient}
 ...
 در حالی که \lr{DDPG} گاهی اوقات می‌تواند عملکرد بسیار خوبی داشته باشد، اما اغلب نسبت به ابرپارامترها و سایر انواع تنظیمات یادگیری حساس است.
  یک حالت رایج شکست یادگیری عامل در \lr{DDPG} این است که تابع \lr{Q} یادگرفته شده شروع به بیش برآورد مقادیر \lr{Q} می‌کند که منجر به واگرایی سیاست می‌شود. واگرایی به این دلیل زخ می‌دهد که در فرایند یادگیری سیاست از تخمین تابع \lr{Q}
...
%  		$a = \text{clip}(\mu_{\theta}(s) + \epsilon, a_{Low}, a_{High})$
  		...
  		\State تابع \lr{Q} را با یک قدم نزول گرادیان با استفاده از رابطه زیر به‌روزرسانی کنید:
  		\begin{align*} 
