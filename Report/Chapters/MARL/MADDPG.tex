...
در محیط‌های چندعاملی، رفتار سایر عامل‌ها هم‌زمان با فرایند یادگیری در حال تغییر است؛ بنابراین توزیع انتقال $p(s'|s,a)$ از دید هر عامل ثابت باقی نمی‌ماند. در نتیجه، محیط از منظر عامل «غیراستا» تلقی می‌شود، زیرا یکی از فرض‌های بنیادی در روش‌های یادگیری تقویتی تک‌عاملی مانند DDPG، \textbf{ثبات دینامیک محیط در طول یادگیری} است. نقض این فرض باعث ناپایداری در برآورد تابع ارزش، انحراف در به‌روزرسانی گرادیان سیاست و در نهایت همگرایی ضعیف یا شکست فرایند یادگیری می‌گردد.

برای مقابله با این مسئله، چارچوب MA-DDPG از رویکرد \lr{Centralized Training with Decentralized Execution (CTDE)} بهره می‌گیرد. در مرحله آموزش، هر عامل علاوه بر مشاهدات محلی خود، به اطلاعات حالت و اقدامات سایر عامل‌ها نیز دسترسی دارد و منتقد مرکزی (Centralized Critic) تابع ارزش مشترکی را تقریب می‌زند:
\[
Q_i = Q_i(s, a_1, a_2, \dots, a_N)
\]
این ساختار به منتقد اجازه می‌دهد \textbf{تغییرات سیاست سایر عامل‌ها را در مدل ارزش منعکس کرده و نوسانات ناشی از غیراستایی محیط را کاهش دهد}. بدین ترتیب، تابع ارزش در طول یادگیری نسبت به تغییر رفتار عامل‌های دیگر حساسیت کمتری خواهد داشت.
...
مزیت اصلی CTDE در MA-DDPG را می‌توان به صورت زیر خلاصه کرد:
\begin{itemize}
...
شواهد تجربی پژوهش حاضر نیز نشان می‌دهد که نسخه MA-DDPG نسبت به DDPG تک‌عاملی در سناریوهای دارای اغتشاشات تصادفی و عدم‌قطعیت‌های مدل، از پایداری و همگرایی به‌مراتب بهتری برخوردار است.
