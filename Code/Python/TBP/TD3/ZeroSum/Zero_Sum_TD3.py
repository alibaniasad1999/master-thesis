...
class ZS_TD3:
    def __init__(
        ...
        self.first_player_start_learning_epoch = first_player_start_learning_epoch
        ...
        # Create actor-critic module and target networks for second player
        self.ac_1 = self.actor_critic(self.env.observation_space, self.env.action_space, **self.ac_kwargs)
        ...
        self.pi_optimizer_1 = Adam(self.ac_1.pi.parameters(), lr=self.pi_lr)
        self.q_optimizer_1 = Adam(self.q_params_1, lr=self.q_lr)
    ...
    def compute_loss_q_1(self, data):
        ...
        # Useful info for logging
        loss_info = dict(Q1Vals_1=q1.detach().numpy(),
                         Q2Vals_1=q2.detach().numpy())
    ...
    #
    #
    def update(self, data, timer, first_player=False):
        # First run one gradient descent step for Q1 and Q2
        if first_player:
            ...
            loss_q = np.array([0])
            loss_info = dict(Q1Vals=0, Q2Vals=0)
        ...
        loss_q, loss_info = self.compute_loss_q_1(data)
        loss_q.backward()
        self.q_optimizer_1.step()

        # Record things
        self.logger.store(LossQ_1=loss_q.item(), **loss_info)
        ...
        if timer % self.policy_delay == 0:
            ...
            loss_pi = self.compute_loss_pi_1(data)
            loss_pi.backward()
            self.pi_optimizer_1.step()

            # Record things
            self.logger.store(LossPi_1=loss_pi.item())
    ...
    def get_action_1(self, o, noise_scale):
        a = self.ac_1.act(torch.as_tensor(o, dtype=torch.float32))
        a += noise_scale * np.random.randn(self.act_dim)
        return np.clip(a, -self.act_limit, self.act_limit)
    ...
    def train(self):
        ...
        for t in range(total_steps):
            ...
            a = a = self.get_action(o, self.act_noise)
            if t > self.start_steps:
                a_1 = self.get_action_1(o, self.act_noise)
            else:
            ...
            if t >= self.update_after and t % self.update_every == 0:
                first_player = self.steps_per_epoch * self.first_player_start_learning_epoch < t
                if first_player:
                    print(colorize("First player is updating", 'blue', bold=True))
                else:
                    print(colorize("First is not updating", 'red', bold=True))
                for j in range(self.update_every):
                    batch = self.replay_buffer.sample_batch(self.batch_size)
                    self.update(data=batch, timer=j, first_player=first_player)
    ...
    def save_1(self, filepath='model/'):
        ...
        # Check if model is on CUDA or CPU
        device_type = next(self.ac_1.pi.parameters()).device.type
        if device_type == 'cuda':
        ...
        print(colorize(f"Second player model saved successfully! 🥰😎", 'blue', bold=True))
