# -*- coding: utf-8 -*-
"""TD3_des_new_format.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Ni_D0O7685lJWFAoDa8TjW3JAzrt1PWS
"""
...
class MassSpringDamperEnv(gym.Env):
    ...
    def reset(self,
        ...
        self.current_step = 0
        self.error = np.concatenate([np.array([self.state[0] - self.desired.item()]), np.array([self.state[1]])])
        return self.error
    ...
    def step(self, action):
        ...
        reward = -np.linalg.norm(self.error[0])

        # Check if the episode is done
        done = self.current_step >= self.max_steps


        return self.error, reward, done, {}
...
while not done:
    ...
    state_array.append(obs + np.array([env.desired.item(), 0]))
    action_array.append(action)
    desired_array.append(env.desired)
    episode_reward += reward
    env.render()
print(f"Episode Reward: {episode_reward:.3f}")
position_array = [x[0] for x in state_array]
plt.plot(np.linspace(0, 10, num=len(state_array)),position_array)
plt.plot(np.linspace(0, 10, num=len(state_array)),desired_array)
plt.xlabel("Time (sec)")
plt.ylabel("State")
plt.legend(["Position", "desired"])
plt.show()
...
while not done:
    ...
    desired_array.append(env.desired)
    state_array.append(state + np.array([env.desired.item(), 0]))
    action_array.append(action)
    episode_reward += reward
    env.render()
print(f"Episode Reward: {episode_reward:.3f}")
