...
# three body problem env with initial condition shift
class ThreeBodyEnv:
    def __init__(self, trajectory_, error_range=0.1, final_range=0.1, dt=0.01,
                 shift_std=0.05, obs_noise_std=0.01):
        ...
        # Initial condition shift and observation noise parameters
    ...
    def step(self, action, action_2=np.zeros(2)):
        ...
        self.state = self._apply_obs_noise(self.state)
    ...
    def _shift_initial_condition(self, initial_state):
        ...
        shifted_state = np.clip(shifted_state, self.observation_low, self.observation_high)
    ...
    def _apply_obs_noise(self, observation):
        ...
        noisy_obs = np.clip(noisy_obs, self.observation_low, self.observation_high)
        return noisy_obs
    ...
    def reset(self,
              *,
              ...
              options: 6 = None):
        # Initialize state to the beginning of trajectory
        self.position = self.trajectory[0].copy()

        # Apply initial condition shift
        self.position = self.position + np.random.normal(0, self.shift_std, size=self.position.shape)

        self.steps = 0
        self.position2state()

        # Apply observation noise to initial state
        self.state = self._apply_obs_noise(self.state)

        return 1000 * self.state, {}


class InitialConditionShiftNode(Node):
    def __init__(self):
        super().__init__('initial_condition_shift_node')
        ...
        self.declare_parameter('shift_std', 0.05)  # Standard deviation for initial condition shift
        self.declare_parameter('obs_noise_std', 0.01)  # Standard deviation for observation noise

        self.time_step = self.get_parameter('time_step').get_parameter_value().double_value
        self.shift_std = self.get_parameter('shift_std').get_parameter_value().double_value
        self.obs_noise_std = self.get_parameter('obs_noise_std').get_parameter_value().double_value

        # Log parameters
        self.get_logger().info(f'time_step: {self.time_step}')
        self.get_logger().info(f'shift_std: {self.shift_std}')
        self.get_logger().info(f'obs_noise_std: {self.obs_noise_std}')

        # Validate the parameters
        if self.time_step <= 0.0:
            self.get_logger().warn(f'Invalid time_step ({self.time_step}). Setting to default 1.0s.')
            self.time_step = 1.0
        if self.shift_std < 0.0:
            self.get_logger().warn(f'Invalid shift_std ({self.shift_std}). Setting to default 0.05.')
            self.shift_std = 0.05
        if self.obs_noise_std < 0.0:
            self.get_logger().warn(f'Invalid obs_noise_std ({self.obs_noise_std}). Setting to default 0.01.')
        ...
        # Initialize environment with initial condition shift and observation noise
        ...
        self.get_logger().info('Initial Condition Shift Node has been started.')
...
def main(args=None):
    ...
    initial_condition_shift_node = InitialConditionShiftNode()
    try:
        rclpy.spin(initial_condition_shift_node)
    except KeyboardInterrupt:
        pass
    finally:
        initial_condition_shift_node.destroy_node()
        rclpy.shutdown()
