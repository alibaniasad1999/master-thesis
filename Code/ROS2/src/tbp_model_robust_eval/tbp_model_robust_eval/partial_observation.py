...
class ThreeBodyEnv:
    def __init__(self, trajectory_, error_range=0.1, final_range=0.1, dt=0.01, observation_mask_ratio=0.0):
        self.trajectory = trajectory_
        self.state = np.zeros(4)
        self.dt = dt/10
        self.mu = 0.012277471
        # Action and observation limits
        self.action_low = -4
        self.action_high = 4
        ...
        # Partial observation parameter
        self.observation_mask_ratio = observation_mask_ratio
        self.reset()
    ...
    def step(self, action, action_2=np.zeros(2)):
        ...
        if self.observation_mask_ratio > 0:
            self.state = self._apply_sensor_loss(self.state)
    ...
    def _apply_sensor_loss(self, observation):
        ...
        # Create a mask for the observation. Randomly set some observation components to zero.
        mask = np.random.rand(*observation.shape) > self.observation_mask_ratio
        masked_observation = observation.copy()
        masked_observation[~mask] = 0  # Set the masked elements to zero to simulate missing data
    ...
    def reset(self,
        ...
        # Apply partial observation (sensor loss) to initial state
        if self.observation_mask_ratio > 0:
            self.state = self._apply_sensor_loss(self.state)
...
class PartialObservationNode(Node):
    def __init__(self):
        super().__init__('partial_observation_node')
        ...
        self.declare_parameter('observation_mask_ratio', 0.5)  # Default 50% sensor loss

        self.time_step = self.get_parameter('time_step').get_parameter_value().double_value
        self.observation_mask_ratio = self.get_parameter('observation_mask_ratio').get_parameter_value().double_value

        # Log parameters
        self.get_logger().info(f'time_step: {self.time_step}')
        self.get_logger().info(f'observation_mask_ratio: {self.observation_mask_ratio}')

        # Validate parameters
        if self.time_step <= 0.0:
            self.get_logger().warn(f'Invalid time_step ({self.time_step}). Setting to default 1.0s.')
            self.time_step = 1.0
        if self.observation_mask_ratio < 0.0 or self.observation_mask_ratio > 1.0:
            self.get_logger().warn(f'Invalid observation_mask_ratio ({self.observation_mask_ratio}). '
                                  f'Setting to default 0.5.')
            self.observation_mask_ratio = 0.5
        ...
        # Initialize environment with partial observation
        self.tbp_env = ThreeBodyEnv(
            trajectory_in,
            dt=self.time_step,
            error_range=0.01,
            final_range=0.001,
            observation_mask_ratio=self.observation_mask_ratio
        )
        ...
        self.get_logger().info('Partial Observation Node has been started.')
        self.get_logger().info(f'Sensor loss ratio: {self.observation_mask_ratio * 100:.1f}%')
    ...
    def timer_callback(self):
        ...
        # Create a request with the current state (which may have some sensors masked out)
        ...
        if self.observation_mask_ratio > 0:
            self.get_logger().info(f'Note: This state has {self.observation_mask_ratio * 100:.1f}% sensor loss applied')
...
def main(args=None):
    ...
    partial_observation_node = PartialObservationNode()
    try:
        rclpy.spin(partial_observation_node)
    except KeyboardInterrupt:
        pass
    finally:
        partial_observation_node.destroy_node()
        rclpy.shutdown()
