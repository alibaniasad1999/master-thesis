...
class ThreeBodyEnv:
    def __init__(self, trajectory_, error_range=0.1, final_range=0.1, dt=0.01, noise_std=0.05):
    ...
    def step(self, action, action_2=np.zeros(2)):
        ...
        # Apply sensor noise to the state
        noisy_state = self._add_sensor_noise(self.state)
        ...
        return 1000 * noisy_state, reward, done, False, self.position
    ...
    def _add_sensor_noise(self, observation):
        """
        Adds Gaussian noise to simulate noisy sensor readings.
        """
        if self.noise_std <= 0:
            return observation
        ...
        return np.clip(noisy_obs, self.observation_low, self.observation_high)
    ...
    def reset(self,
        ...
        # Apply sensor noise to initial state
        noisy_state = self._add_sensor_noise(self.state)

        return 1000 * noisy_state, {}
...
class SensorNoiseNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('noise_std', 0.05)  # Standard deviation for sensor noise
        ...
        if self.noise_std < 0.0:
            self.get_logger().warn(f'Invalid noise_std ({self.noise_std}). Setting to default 0.05.')
            self.noise_std = 0.05
        ...
        # Initialize environment with sensor noise
        ...
        self.get_logger().info('Sensor Noise Node has been started.')
        self.get_logger().info(f'Using sensor noise standard deviation: {self.noise_std}')
    ...
    def timer_callback(self):
        ...
        # Create a request with the current state (which includes sensor noise)
        ...
        self.get_logger().info(f'Sending noisy state -> Position: ({self.position_x}, {self.position_y}), Velocity: ({self.velocity_x}, {self.velocity_y})')
...
def main(args=None):
    ...
    sensor_noise_node = SensorNoiseNode()
    try:
        rclpy.spin(sensor_noise_node)
    except KeyboardInterrupt:
        pass
    finally:
        sensor_noise_node.destroy_node()
        rclpy.shutdown()
