...
class ThreeBodyEnv:
    def __init__(self, trajectory_, error_range=0.1, final_range=0.1, dt=0.01,
                 delay_steps=3, disturbance_std=0.05):
        ...
        self.action_buffer = []
        self.last_action = np.zeros(2)  # Initialize with zero action
    ...
    def step(self, action, action_2=np.zeros(2)):
        ...
        if len(self.action_buffer) < self.delay_steps:
            # Buffer still filling up, use last action or zeros if no previous action
            self.action_buffer.append(action)
            action_to_execute = self.last_action
        else:
            # Execute delayed action from buffer
            action_to_execute = self.action_buffer.pop(0)
            self.action_buffer.append(action)
            self.last_action = action_to_execute

        # Apply actuator noise/disturbance
        if self.disturbance_std > 0:
            noise = np.random.normal(0, self.disturbance_std, size=action_to_execute.shape)
            action_to_execute = np.clip(action_to_execute + noise, self.action_low, self.action_high)
        ...
        action_to_execute = np.clip(action_to_execute, self.action_low, self.action_high)
        action_2 = np.clip(action_2, self.action_low, self.action_high)

        a_x = action_to_execute[0] / 100
        a_y = action_to_execute[1] / 100
    ...
    def reset(self,
        ...
        # Reset action buffer and last_action
        self.action_buffer = []
        for _ in range(self.delay_steps):
            self.action_buffer.append(np.zeros(2))  # Fill buffer with zero actions
...
class TimeDelayNode(Node):
    def __init__(self):
        super().__init__('time_delay_node')
        ...
        self.declare_parameter('delay_steps', 3)  # Default number of steps to delay actions
        self.declare_parameter('disturbance_std', 0.05)  # Default standard deviation for actuator noise
        ...
        self.get_logger().info(f'delay_steps: {self.delay_steps}')
        ...
        if self.delay_steps < 0:
            self.get_logger().warn(f'Invalid delay_steps ({self.delay_steps}). Setting to default 3.')
            self.delay_steps = 3
        ...
        # Initialize environment with time delay parameters
        ...
        self.get_logger().info('Time Delay Node has been started.')
        self.get_logger().info(f'Action delay: {self.delay_steps} steps')
        self.get_logger().info(f'Actuator noise std: {self.disturbance_std}')
    ...
    def handle_service_response(self, future):
        try:
            ...
            if self.delay_steps > 0:
                self.get_logger().info(f'Note: This control action will be delayed by {self.delay_steps} steps')
...
def main(args=None):
    ...
    time_delay_node = TimeDelayNode()
    try:
        rclpy.spin(time_delay_node)
    except KeyboardInterrupt:
        pass
    finally:
        time_delay_node.destroy_node()
        rclpy.shutdown()
